function [ FI0 ] = serothbesselfunct(theta1)
    %SEROTHBESSELFUNCT: this part of the code is in-charge of the integral with the seroth bessel function
    ...
    FI0 = sqrt(cos(theta1(:))).*sin(theta1(:)).*exp(1i*k0*psy(:)).*(TN1(:,1)+TN1(:,2).*cos(theta(:,M))).*besselj(0,k(1).*sin(theta1(:))*rp*sin(thetap)).*exp(1i*k(M)*rp*cos(thetap).*cos(theta(:,M)));
end
